...
if [ ! -f "news.2012.en.shuffled" ]; then
  ...
  echo 'Filling data from news.2012.en.shuffled'
...
if [ ! -f "news.2013.en.shuffled" ]; then
  ...
  echo 'Filling data from news.2013.en.shuffled'
...
if [ ! -d "1-billion-word-language-modeling-benchmark-r13output" ]; then
  ...
  echo 'Filling data from 1-billion-word-language-modeling-benchmark'
...
if [ ! -f "enwiki-latest-pages-articles.xml.bz2" ]; then
  ...
  echo 'Filling data from enwiki-latest-page'
...
./word2vec -train data-phrase2.txt -output vectors.bin -window 10 -negative 10 -sample 1e-5 -min-count 10
